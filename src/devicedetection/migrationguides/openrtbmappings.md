...
|sua|Structured user agent information defined by a UserAgent object (see Section 3.2.29 of the OpenRTB 2.6 spec).|Info obtained from the [User-Agent Client Hints (UA-CH) HTTP headers](@ref DeviceDetection_Features_UACH_Overview) (or equivalent NavigatorUAData JS object) sent by the browsers that support it.|ua may contain a reduced User-Agent string, thus if present 'sua' should be considered a more accurate and complete device info. [You can also convert UA-CH values into sua information](https://51degrees.com/blog/openrtb-structured-user-agent-client-hints) with our [sua and UA-CH converter](https://51degrees.github.io/sua-uach-conversion/).|
