...
- Low Memory: memory overhead is reduced due to the use of hash values rather than the characters of the substrings.
...
# Data set production <a href="#DeviceDetection_Hash_DataSetProduction">#</a> @anchor DeviceDetection_Hash_DataSetProduction
...
## Performance graphs <a href="#DeviceDetection_Hash_DataSetProduction_Performance">#</a> @anchor DeviceDetection_Hash_DataSetProduction_Performance

**⚠️ Deprecated:** Performance graphs are deprecated since v4.5 and are now just an alias of the predictive graph.

Performance optimized graphs are constructed using decision trees which result in the fastest detection for a given evidence string. Generally, this results in nodes where a long hash is looked for in a narrow range. For example, if the substring `Android 4.0.4` is known to exist at character positions 18-25 within the training data, why look for it in a larger range which would be slower? Using the exact position `Android 4.0.4` at character 20 reduces the possible devices very quickly, and evaluation of the exact position is quicker than evaluation of a range. This approach works very well with the training data, but how does it work on evidence which has not been seen before? Consider a User-Agent containing the substring `iPhone 8` which is in the training data, then along comes one containing the substring `iPhone 8s`. All substrings to the right of this will be shifted by one character. If the next node is looking for `Version` starting at character 120, it now needs to be found at character 121. Because of the very narrow range used in a performance graph, the string would not be matched. Predictive graphs help to solve this problem, so it's unusual to use performance graphs in isolation.

## Predictive graphs <a href="#DeviceDetection_Hash_DataSetProduction_Predictive">#</a> @anchor DeviceDetection_Hash_DataSetProduction_Predictive
...
# Predictive power <a href="#DeviceDetection_Hash_PredictivePower">#</a> @anchor DeviceDetection_Hash_PredictivePower
...
To allow for drift in substrings within a User-Agent, the drift tolerance can be set. In the case of the above example, setting the drift to one would mean that even though the whole User-Agent was shifted by one character position, it would be matched as the original User-Agent.

This feature can also be useful in predicting new devices which have not yet been seen in the wild. For example, if a User-Agent containing the substring `iPhone 8` was known, then along came one containing the substring `iPhone 8s`, all substrings to the right of this would be shifted by one character. Setting the drift allows the Browser and OS to be correctly identified even though the substrings used to identify them are not in the position they are expected to be in.
...
Take the example User-Agent (assuming we are using a performance graph) for a known browser version which contains the substring `Android 4.0.4`, and an unknown version containing the substring `Android 4.0.5`. Due to the nature of the chosen **hash** algorithm, the hashes of the two substrings vary proportionally to the difference in ASCII value of the last character. This means that if the difference is set to one, the unknown substring will be allowed as a match for the known Android version.

# The algorithm in action <a href="#Device_Detection_Hash_InAction">#</a> @anchor Device_Detection_Hash_InAction
Now that you've got an idea of how the algorithm and data structure work, why not try our [User-Agent Tester](https://51degrees.com/developers/user-agent-tester) to see the algorithm in action. Or try it for yourself with our [Getting Started](@ref DeviceDetection_Examples_GettingStarted_Console_OnPremise) examples.
