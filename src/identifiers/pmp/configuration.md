...
| `data-timeout` | No | `1500` | Milliseconds the platform allows for three things, being the read of the group's shared answer when the page loads, the write when a visitor shares an answer, and the one wait there can be between the two cards. That wait happens only where the client script has not yet called `complete` when the visitor answers the first card, which nearly always means the platform added the script itself and it is still finishing its first round. During that wait the cards are darkened under the waiting ring and nothing on them can be pressed, and when the time runs out the platform offers the second card as it would for any result that is not known. Ordinarily the client script has finished long before the visitor answers, and then nothing waits. See @ref Identifiers_PMP_Sharing. A value that is not a positive whole number is warned about and ignored, and anything above 4500 is capped, because in each of the three waits the visitor can do nothing until it ends. |
